...
TEST(TypedEquationNode, type_int){
    ...
    auto nodeMain = TypedEquationNode::make_node(0, EquationResolver::Rule::sum, nodeLeft, nodeRight);
    ASSERT_EQ(nodeMain->get<int>(), 3);
    ASSERT_EQ(nodeMain->get<double>(), 3.);
}

TEST(TypedEquationNode, type_double){
    auto nodeLeft = TypedEquationNode::make_node(1.5, EquationResolver::Rule::variable);
    auto nodeRight = TypedEquationNode::make_node(2., EquationResolver::Rule::variable);
    auto nodeMain = TypedEquationNode::make_node(0., EquationResolver::Rule::sum, nodeLeft, nodeRight);
...
TEST(TypedEquationNode, type_deduction_recalculating){
    ...
    nodeRight->setValue(6.5);
    ASSERT_EQ(nodeMain->get<double>(), 9.5);
...
TEST(TypedEquationNode, node_appending){
    ...
    auto nodeMainMain = TypedEquationNode::make_node(0., EquationResolver::Rule::minus, nodeMain, nodeMainRight);
